...
class Base(LanNannyClient):
    ...
    def get_by_field(self, field_name: str, field_value):
        ...
        request = self.make_request(url="device-mac", method="GET", payload=request_args)
        ...
        response = self.make_request(self.model_url, payload=payload)
        import ipdb; ipdb.set_trace()
        if response["status"] == "success":
            return self.build(response["object"])
        else:
...
# End File: politeauthority/lan-nanny/src/lan_nanny/client/models/base.py
